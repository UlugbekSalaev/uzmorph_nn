...
class uzmorph_bilstm:
    def __init__(self, model_path=None, char_vocab=None, tag_vocab=None):
        # Default paths pointing to package data
        if model_path is None:
            model_path = pkg_resources.resource_filename('uzmorph_bilstm', 'data/morph_bilstm.pth')
        if char_vocab is None:
            char_vocab = pkg_resources.resource_filename('uzmorph_bilstm', 'data/char_vocab.json')
        if tag_vocab is None:
            tag_vocab = pkg_resources.resource_filename('uzmorph_bilstm', 'data/tag_vocab.json')
        ...
        self.model = MorphBiLSTM(len(self.char_to_idx), len(self.tag_to_idx))
